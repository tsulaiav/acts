...
BOOST_AUTO_TEST_CASE(DD4hepCylidricalDetectorExplicit) {
  ...
  cylinder.addMaterial("Pixel_Material", [&](auto& mat) {
    ...
    for(int ecid : {-1, 1}) {
      ...
      ecGeoId.setAllVolumeIdsTo(s_pixelVolumeId+ecid)
        .incrementLayerIds(1);
      ...
      std::map<int, std::vector<std::shared_ptr<Acts::Surface>>> initialLayers{};
      const DetElement* pixelEndcapElement = ecid==1 ? find_element(*pixelElement, "PixelPositiveEndcap")
      ...
      for (const auto& [nameLayer, layer] : pixelEndcapElement->children()) {
        for (const auto& [nameModule, module] : layer.children()) {
          ...
          auto dd4hepDetEl = std::make_shared<Acts::DD4hepDetectorElement>(
              module, detAxis, 1_cm, false, nullptr);
          detectorElements.push_back(dd4hepDetEl);
          initialLayers[layerId].push_back(dd4hepDetEl->surface().getSharedPtr());
        }
        layerId++;
      }
      ...
      for (const auto& [key, surfaces] : initialLayers) {
        auto& layer = protoLayers.emplace_back(Acts::GeometryContext(), surfaces);
      ...
      std::vector<LayerData> mergedLayers = mergeLayers(Acts::GeometryContext(), protoLayers);
